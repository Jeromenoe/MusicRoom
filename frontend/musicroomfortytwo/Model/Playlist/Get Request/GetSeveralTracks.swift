...
// Structure that we are going to use to display a playlist
struct DisplayablePlaylist {
    ...
    let imageUrl: String
    let artists: String
    // add back end ids ! ON PEUT ENELVER !!
    // add vote ?
}
...
class GetSeveralTracks {
    ...
    // Need to be call just before the user delete a track in a playlist
...
//        self.printDisplayablePlaylist()
    ...
    func printDisplayablePlaylist() {
        ...
        let count = displayablePlaylist.count
        if count > 0 {
            for i in 0...(count - 1) {
                print("Son n°", String(i), " titre : ", displayablePlaylist[i].name, " id : ", displayablePlaylist[i].id)
            }
        }
    }
}
