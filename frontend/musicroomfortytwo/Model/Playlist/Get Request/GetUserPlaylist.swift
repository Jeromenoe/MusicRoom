...
/*
        Structures needed to accomplish the Request
 */
...
class GetUserPlaylist {
    ...
    func getUserPlaylist(callback: @escaping (Int) -> Void) {
        ...
        // ATTENTION : il faut gérer le cas où le token a expiré ! Le token n'est généré que lors de la connexion, il faut pouvoir en générer sans que le user ait à se déconnecter / reconnecter.
        ...
        task = session.dataTask(with: request) { (data, response, error) in
            DispatchQueue.main.async {
                ...
                do {
                    // Ajouter message d'erreur en fonction du statusCode
