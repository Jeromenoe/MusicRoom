...
class GetFollowersOfAPlaylist {
    ...
    func printResult() {
        var count = self.followers.count
        if count > 0 {
            for i in 0...(count - 1) {
                print("follower : ", String(i + 1), " pseudo : ", self.followers[i].userPseudo, " id : ", self.followers[i].userId)
            }
        }
        count = self.associatedUser.count
        if count > 0 {
            for i in 0...(count - 1) {
                print("associé : ", String(i + 1), " pseudo : ", self.associatedUser[i].userPseudo, " id : ", self.associatedUser[i].userId, " edition right : ", self.associatedUser[i].editionRight)
            }
        }
    }
