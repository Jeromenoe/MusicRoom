const { response, eventNames } = require('../app');
...
async function checkIfTrackShouldTerminate(event) {
    let endSongDate = new Date(event.tracksInfo[0].timeBeginListening.getTime() + event.tracksInfo[0].trackDuration)
    if ((endSongDate - 2000) <= Date.now()) {
        console.log("track is finished")
        await deleteTrackFromEventData(event._id.toString(), event.tracksInfo[0].trackId);
...
async function getAllStartedEvent() {
    let events = await Event.find( { $or: [ {status: "started"}, {beginDate: {$lte: Date.now()}, endDate: {$gt: Date.now()}} ] })
...
async function addInfosOnUserEvents(req) {
    let user = await User.findOne({_id: getUserIdFromToken(req)})
        ...
        for (let i = 0; user.userData.events[i]; i++) {
            user.userData.events[i].eventsId.creator = user.userData.events[i].eventsId.creator.userInfo.pseudo
        ...
        return user.userData.events.map(function(e) { return e.eventsId; });
}
...
exports.getSpecifiedEvent = async(req, res, next) => {
    try {
        ...
        if (event.publicFlag == false) {
            if (await getUserEventAccess(getUserIdFromToken(req), req.params.eventId) == -1)
...
{
    if (!req.params.eventId || req.params.eventId.length != 24 || !/^[a-f0-9]+$/.test(req.params.eventId))
        throw new Error("Invalid event id!");
}
...
exports.getUserRightFromSpecificEvent = async(req, res, next) => {
    try {
        ...
        let userRight = await getUserEventAccess(req.params.userId, req.params.eventId)
...
exports.getAllUserEvents = async(req, res, next) => {
    try {
        ...
        .populate('userData.events.eventsId');
        let eventList = user.userData.events.map(function(e) { return e.eventsId; });
        res.musicRoomMessage = "Ok"
        res.status(200).json({
            code: '0',
            events: eventList
...
async function addVotedTracksInArr(req, event)
{
    ...
    let eventIdx = user.userData.events.map(function(e) { return e.eventsId; }).indexOf(event._id)
    if (eventIdx == -1 && event.publicFlag == false)
        throw new Error("User cannot access private event!");
    event.tracksInfo.forEach(element => {
        if (eventIdx == -1) {
            element.userVote = false
        } else if (user.userData.events[eventIdx].songIdVotes.indexOf(element.trackId) == -1) {
            element.userVote = false
        } else {
            element.userVote = true
        }
...
exports.getSortedTrackList = async(req, res, next) => {
    try {
        ...
        if (!event)
            throw new Error('Invalid event id!');
        event = event.toObject()
        event.tracksInfo.sort((a, b) => { return b.votesNumber - a.votesNumber; });
...
async function addEventToUserData(userId, event)
{
    ...
    if (user.userData.events.map(function(e) { return e.eventsId; }).indexOf(event._id) != -1)
...
function parseCreateEventArg(req)
{
    if (!req.body.name || !/^[a-z0-9éèàêô,'! -]+$/i.test(req.body.name))
    ...
    else if (req.body.visibility && req.body.visibility != 'public' && req.body.visibility != 'private')
        throw new Error('Invalid visibility format!');
    else if (req.body.votingPrerequisites && req.body.votingPrerequisites != 'true' && req.body.votingPrerequisites != 'false')
        throw new Error('Invalid votingPrerequisites format!');
    else if (req.body.physicalEvent && req.body.physicalEvent != 'true' && req.body.physicalEvent != 'false')
        ...
        for (let i = 0; i < musicalPrefArr.length; i++) {
            for (let j = 0; j < musicalStylesAllowed.length; j++) {
                if (musicalPrefArr[i] == musicalStylesAllowed[j])
                    break;
    ...
    if (req.body.physicalEvent && req.body.physicalEvent == 'true') {
        ...
        else if (!req.body.beginDate || !/(^\d{4}-([0]\d|1[0-2])-([0-2]\d|3[01])$)|(\d{4}-[01]\d-[0-3]\dT[0-2]\d:[0-5]\d:[0-5]\d\.\d+([+-][0-2]\d:[0-5]\d|Z))|(\d{4}-[01]\d-[0-3]\dT[0-2]\d:[0-5]\d:[0-5]\d([+-][0-2]\d:[0-5]\d|Z))|(\d{4}-[01]\d-[0-3]\dT[0-2]\d:[0-5]\d([+-][0-2]\d:[0-5]\d|Z))/.test(req.body.beginDate)) 
            throw new Error('Invalid beginDate format!');
        else if (!req.body.endDate || !/(^\d{4}-([0]\d|1[0-2])-([0-2]\d|3[01])$)|(\d{4}-[01]\d-[0-3]\dT[0-2]\d:[0-5]\d:[0-5]\d\.\d+([+-][0-2]\d:[0-5]\d|Z))|(\d{4}-[01]\d-[0-3]\dT[0-2]\d:[0-5]\d:[0-5]\d([+-][0-2]\d:[0-5]\d|Z))|(\d{4}-[01]\d-[0-3]\dT[0-2]\d:[0-5]\d([+-][0-2]\d:[0-5]\d|Z))/.test(req.body.endDate))
...
async function createEventFromBody(req)
{
    ...
    if (req.body.physicalEvent == 'true') {
        ...
        event = copyBodyParamsToNumericalEvent(req);
    ...
    if (!event)
        throw new Error('Error: Invalid event id!');
    return event;
}
...
async function convertAddressToGeoLoc(req)
{
    ...
    if (!data[0])
        throw new Error('Cannot find place location!');
...
function parseSetStatusEventArg(req)
{
    if (!req.params.eventId || req.params.eventId.length != 24 || !/^[a-f0-9]+$/.test(req.params.eventId))
        throw new Error("Invalid event id!");
    if (!req.body.eventStatus || (req.body.eventStatus != 'notStarted' && req.body.eventStatus != 'started' && req.body.eventStatus != 'terminated'))
...
exports.setEventStatus = async(req, res, next) => {
    try {
        ...
        if (await getUserEventAccess(getUserIdFromToken(req), req.params.eventId) != 2)
            throw new Error("User should be admin to set event status!")
        let event = await Event.findOne({_id: req.params.eventId});
        if (!event)
            throw new Error("Error: Invalid eventId!");
...
async function deleteEventFromUserData(userId, eventId)
{
    ...
    if (!user)
        throw new Error('Invalid user id!');
    let eventPos = user.userData.events.map(function(e) { return e.eventsId; }).indexOf(eventId);
    if (eventPos == -1)
        throw new Error('Invalid event id!');
...
async function deleteUserFromEvent(userId, eventId)
{
    ...
    if (!event)
        throw new Error('Invalid event id!');
    let userPos = event.guestsInfo.map(function(e) { return e.userId; }).indexOf(userId);
    if (userPos == -1)
...
async function getNumberOfEventAdmin(eventId)
{
    ...
    if (!event)
        throw new Error('Invalid event id!');
    usersRights = event.guestsInfo.map(function(e) { return e.right; });
    let nbAdmin = 0;
...
exports.leaveEvent = async (req, res, next) => { //Should delete user vote when leaving an event
    try {
...
function parseInviteFriendErrors(res, err)
{
    ...
    } else if (err.message == "User should be admin to invite friend to private event!") {
        res.status(400).json({
            code: '3',
            message: err.message
        });
    } else if (err.message == "Users must be friends to invite each other to the event!") {
        res.status(400).json({
            code: '4',
            message: err.message
        });
    } else if (err.message == "Friend already has a pending invitation for this event!") {
        res.status(400).json({
            code: '5',
            message: err.message
        });
...
{
    if (!req.params.friendId || req.params.friendId.length != 24 || !/^[a-f0-9]+$/.test(req.params.friendId))
        throw new Error("Invalid friend id!");
    else if (req.params.friendId == req.params.userId)
        throw new Error("Cannot invite yourself to event!");
    else if (!req.params.eventId || req.params.eventId.length != 24 || !/^[a-f0-9]+$/.test(req.params.eventId))
        throw new Error("Invalid event id!");
    else if (req.body.friendRight && (req.body.friendRight != 'guest' && req.body.friendRight != 'superUser' && req.body.friendRight != 'admin'))
...
function getUserEventAccess_noFind(userId, event)
{
    if (!event)
        throw new Error('Invalid event id!');
    const userPos = event.guestsInfo.map(function(e) { return e.userId.toString(); }).indexOf(userId);
    if (userPos == -1)
        return -1;
    else if (event.guestsInfo[userPos].right == 'guest')
        return 0;
    else if (event.guestsInfo[userPos].right == 'superUser')
        return 1;
    else if (event.guestsInfo[userPos].right == 'admin')
        return 2;
    else
        throw new Error('Error: Invalid event user right!');
}
...
async function getUserEventAccess(userId, eventId)
{
    ...
    if (!event)
        throw new Error('Invalid event id!');
    const userPos = event.guestsInfo.map(function(e) { return e.userId; }).indexOf(userId);
    if (userPos == -1)
        return -1;
    else if (event.guestsInfo[userPos].right == 'guest')
        return 0;
...
async function inviteFriendToEvent(userId, friendId, eventId, userAccess, friendRight)
{
    let event = await Event.findOne({ _id: eventId });
    ...
    if (!friendRight)
        friendRight = 'guest';
    if (friend.userData.notifications.eventsInvitations.map(function(e) { return e.event; }).indexOf(eventId) != -1)
        throw new Error('Friend already has a pending invitation for this event!');
    if (friend.userData.events.map(function(e) { return e.eventsId; }).indexOf(eventId) != -1)
...
exports.inviteFriend = async (req, res, next) => {
    try {
        ...
        let userAccess = await getUserEventAccess(req.params.userId, req.params.eventId);
        await inviteFriendToEvent(req.params.userId, req.params.friendId, req.params.eventId, userAccess, req.body.friendRight);
        res.musicRoomMessage = "Ok"
        res.status(200).json({
            code: '0',
...
async function acceptInvitationToEvent(userId, friendId, eventId, friendAccess)
{
    let friend = await User.findOne({ _id: friendId });
    let event = await Event.findOne({ _id: eventId});
    let user = await User.findOne({ _id: userId });
    ...
    if (!event)
        throw new Error('Invalid event id!');
    let eventInvitationIdx = user.userData.notifications.eventsInvitations.map( function(e) { return e.event }).indexOf(eventId);
    if (eventInvitationIdx == -1 || user.userData.notifications.eventsInvitations[eventInvitationIdx].friend != friendId)
        throw new Error('There is no pending invitation from this user!');
    let userRight = user.userData.notifications.eventsInvitations[eventInvitationIdx].userRight;
...
exports.acceptInvitation = async (req, res, next) =>  {
    try {
        ...
        let friendAccess = await getUserEventAccess(req.params.friendId, req.params.eventId);
        await acceptInvitationToEvent(req.params.userId, req.params.friendId, req.params.eventId, friendAccess);
        res.musicRoomMessage = "Ok"
        res.status(200).json({
            code: '0',
...
async function refuseInvitationToEvent(userId, eventId, friendId)
{
    ...
    if (!event)
        throw new Error('Invalid event id!');
    let eventIdx = user.userData.notifications.eventsInvitations.map( function(e) { return e.event }).indexOf(eventId);
    if (eventIdx == -1 || user.userData.notifications.eventsInvitations[eventIdx].friend != friendId)
...
exports.refuseInvitation = async (req, res, next) => {
    try {
        parseAcceptInvitationEventArg(req);
        await refuseInvitationToEvent(req.params.userId, req.params.eventId, req.params.friendId);
        res.musicRoomMessage = "Ok"
        res.status(200).json({
            code: '0',
...
function parseAddTrackToEventArg(req)
{
    if (!req.params.eventId || req.params.eventId.length != 24 || !/^[a-f0-9]+$/.test(req.params.eventId))
        throw new Error("Invalid event id!");
    else if (!req.params.trackId) //Have to check what is the Spotify trackId format. And if backend can do a validation of track existence in spotify. ()
...
async function findTrackInEvent(eventId, trackId)
{
    ...
    if (!event)
        throw new Error('Invalid event id!');
    return event.tracksInfo.map(function(e) { return e.trackId; }).indexOf(trackId);
}
...
exports.addTrackToEvent = async (req, res, next) => {
    try {
        ...
        await addTrackToEventData(req.params.eventId, req.params.trackId, req.body.trackDuration);
        res.musicRoomMessage = "Ok"
        res.status(200).json({
            code: '0',
...
async function deleteTrackFromUserData(eventId, trackId)
{
    ...
    for (let i = 0; i < event.guestsInfo.length; i++) {
        ...
        if (!user)
            throw new Error('Error: An user should have been found!');
        let eventIndex = user.userData.events.map(function(e) { return e.eventsId }).indexOf(eventId);
        if (eventIndex == -1)
            throw new Error('Error: An event should have been found!');
        let trackIndex = user.userData.events[eventIndex].songIdVotes.indexOf(trackId);
        if (trackIndex != -1) {
...
async function deleteTrackFromEventData(eventId, trackId) {
    ...
    if (!event)
        throw new Error('Invalid event id!');
    let trackIndex = event.tracksInfo.map(function(e) { return e.trackId; }).indexOf(trackId);
    if (trackIndex == -1)
        throw new Error('The track is not in the event!');
...
function parseDeleteTrackFromEventErrors(res, err)
{
    ...
    } else if (err.message == 'User should be admin to delete track from the event!') {
        res.status(400).json({
            code: '2',
            message: err.message
        });
...
function parseVoteTrackEventErrors(res, err)
{
    ...
    } else if (err.message == 'User should join event before voting for tracks!') {
        res.status(400).json({
            code: '3',
            message: err.message
        });

    } else if (err.message == 'User should get special right to vote for tracks in this event!') {
        res.status(400).json({
            code: '4',
            message: err.message
        });
...
function parseVoteTrackEventArg(req)
{
    if (!req.params.eventId || req.params.eventId.length != 24 || !/^[a-f0-9]+$/.test(req.params.eventId))
        ...
        throw new Error('Invalid track id!');
    else if (req.body.lat && !/^[-+]?([1-8]?\d(\.\d+)?|90(\.0+)?)$/.test(req.body.lat))
        throw new Error('Invalid coordinates format!');
    else if (req.body.long && !/^[-+]?(180(\.0+)?|((1[0-7]\d)|([1-9]?\d))(\.\d+)?)$/.test(req.body.long))
        throw new Error('Invalid coordinates format!');
}
...
async function addTrackVoteToEventData(eventId, userId, trackId)
{
    ...
    if (trackIndex == -1)
        throw new Error('Track is not in the event!');
...
async function checkUserVotingPrerequisitesEvent(eventId, userId, req)
{
    ...
    if (event.votingPrerequisites && userRight < 1)
        throw new Error('User should get special right to vote for tracks in this event!');
    if (event.status != 'started') {
        throw new Error('Event should start before participants can vote!')
    }
    if (event.physicalEvent) {
        if (await getUserDistanceFromEvent(req) > 0.250)
        ...
        if (Date.now() > event.endDate)
            throw new Error('Event is terminated!');
    }
}
...
exports.voteTrackEvent = async (req, res, next) => {
    try {
        ...
        await checkUserVotingPrerequisitesEvent(req.params.eventId, getUserIdFromToken(req), req);
        await addTrackVoteToEventData(req.params.eventId, getUserIdFromToken(req), req.params.trackId);
        res.musicRoomMessage = "Ok"
        res.status(200).json({
            code: '0',
...
exports.unvoteTrackEvent = async(req, res, next) => {
    try {
        ...
        await checkUserVotingPrerequisitesEvent(req.params.eventId, getUserIdFromToken(req), req);
        await deleteTrackVoteToEventData(req.params.eventId, getUserIdFromToken(req), req.params.trackId);
        res.musicRoomMessage = "Ok"
        res.status(200).json({
            code: '0',
...
function parseUpdateEventArg(req)
{
    if (!req.params.eventId || req.params.eventId.length != 24 || !/^[a-f0-9]+$/.test(req.params.eventId))
		...
		req.body.musicalPreferences = musicalPrefArr;
	} else if (req.body.status && req.body.status != "notStarted" && req.body.status != "started" && req.body.status != "terminated") {
...
async function updateEventFromArg(req)
{
    ...
    if (req.body.musicalStyle)
        event.musicalStyle = req.body.musicalStyle
...
function parseChangeParticipantRightArg(req)
{
    if (!req.params.eventId || req.params.eventId.length != 24 || !/^[a-f0-9]+$/.test(req.params.eventId))
        throw new Error("Invalid event id!");
    else if (!req.params.participantId || req.params.participantId.length != 24 || !/^[a-f0-9]+$/.test(req.params.participantId))
        throw new Error("Invalid participant id!");
    else if (!req.body.participantRight || (req.body.participantRight != 'delete' && req.body.participantRight != 'guest' && req.body.participantRight != 'superUser' && req.body.participantRight != 'admin'))
...
function parseChangeParticipantRightErrors(res, err)
{
    ...
    } else if (err.message == "User should be admin to change others participants right!") {
        res.status(400).json({
            code: '4',
            message: err.message
        });
...
async function changeParticipantRightInEvent(eventId, userId, participantId, participantRight)
{
    ...
    if (!event)
        throw new Error('Invalid event id!');
    let participantIdx = event.guestsInfo.map(function(e) {return e.userId}).indexOf(participantId);
    if (participantIdx == -1)
        throw new Error('Participant is not in the event!');
    if (participantRight == 'delete')
        await deleteUserFromEvent(participantId, eventId);
    else
        event.guestsInfo[participantIdx].right = participantRight;
    await event.save();
}
...
exports.updateParticipantRight = async(req, res, next) => {
    try {
        ...
        if (await getUserEventAccess(req.params.userId, req.params.eventId) != 2)
            throw new Error('User should be admin to change others participants right!');
        await changeParticipantRightInEvent(req.params.eventId, req.params.userId, req.params.participantId, req.body.participantRight);
        res.musicRoomMessage = "Ok"
        res.status(200).json({
            code: '0',
...
function parseDeleteParticipantErrors(res, err)
{
    ...
    } else if (err.message == "User should be admin to delete participants from event!") {
        res.status(400).json({
            code: '3',
            message: err.message
        });
...
exports.deleteParticipant = async(req, res, next) => {
    try {
        ...
        if (await getUserEventAccess(req.params.userId, req.params.eventId) != 2)
            throw new Error('User should be admin to delete participants from event!');
