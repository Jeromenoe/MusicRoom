...
const CLIENT_ID = "856970011632-pvjlo12sq8ld2vltvbjbqjsoipmdhc19.apps.googleusercontent.com";

const musicalStylesAllowed = ['none', 'blues', 'country', 'disco', 'folk',
...
async function getUsersRelation(userId, targetId)
{
	...
	if (targetUser.userData.friendsId.indexOf(userId) != -1)
		return 1;
	return 0;
}
...
async function getRestrictedUserProfile(userId, targetId)
{
	...
	let targetUser = await User.findOne({ _id: targetId }).select('userInfo.firstName userInfo.lastName userInfo.pseudo\
	...
	targetUser.userData.events.forEach(function(v) { delete v._id;  delete v.songIdVotes});
	targetUser.userData.playlists.forEach(function(v) { delete v._id; delete v.playlistType});
	for (let i = 0; i < targetUser.userData.events.length; i++) {
		if (targetUser.userData.events[i].eventsId.publicFlag == false) {
			if (user.userData.events.map(function(e) {  return e.eventsId }).indexOf(targetUser.userData.events[i].eventsId._id) == -1)
	...
	for (let i = 0; i < targetUser.userData.playlists.length; i++) {
		if (targetUser.userData.playlists[i].playlist.public == false) {
			if (user.userData.playlists.map(function(e) {  return e.playlist }).indexOf(targetUser.userData.playlists[i].playlist._id) == -1)
...
{
	let user = await User.findOne({ _id: userId });
	let targetUser = await User.findOne({ _id: targetId }).select('userInfo.pseudo userData.events userData.playlists userInfo.musicalPreferences -_id')
	.populate('userData.events.eventsId', 'name publicFlag')
	.populate('userData.playlists.playlist', 'name public')
	if (!targetUser)
		throw new Error('Invalid target user id!');
...
exports.getTargetUserProfile = async (req, res, next) => {
	try {
		...
		if (await getUsersRelation(req.params.userId, req.params.targetUserId)) {
			returnedProfile = await getRestrictedUserProfile(req.params.userId, req.params.targetUserId);
			res.musicRoomMessage = "Ok"
			res.status(200).json({
				code: '1',
			...
			returnedProfile = await getPublicUserProfile(req.params.userId, req.params.targetUserId);
			res.musicRoomMessage = "Ok"
			res.status(200).json({
				code: '0',
				targetUser: returnedProfile
...
function parseUpdateUserInformationArg(req)
{
	if (req.body.firstName && !/^[a-zéèê -]+$/i.test(req.body.firstName)) {
	...
	} else if (req.body.birthDate && !/(^\d{4}-([0]\d|1[0-2])-([0-2]\d|3[01])$)|(\d{4}-[01]\d-[0-3]\dT[0-2]\d:[0-5]\d:[0-5]\d\.\d+([+-][0-2]\d:[0-5]\d|Z))|(\d{4}-[01]\d-[0-3]\dT[0-2]\d:[0-5]\d:[0-5]\d([+-][0-2]\d:[0-5]\d|Z))|(\d{4}-[01]\d-[0-3]\dT[0-2]\d:[0-5]\d([+-][0-2]\d:[0-5]\d|Z))/.test(req.body.birthDate)) {
		...
		for (let i = 0; i < req.body.musicalPrefArr.length; i++) {
			for (let j = 0; j < musicalStylesAllowed.length; j++) {
				if (req.body.musicalPrefArr[i] == musicalStylesAllowed[j])
					break;
...
async function copyParamsToUserInfo(req, user)
{
	...
	if (req.body.pseudo) {
		if (req.body.pseudo != user.userInfo.pseudo) {
			let otherUser = await User.findOne({'userInfo.pseudo': req.body.pseudo})
			if (otherUser)
				throw new Error("Pseudo has already been used by another musicRoom user!")
		}
		user.userInfo.pseudo = req.body.pseudo;
	}
	if (req.body.birthDate)
		user.userInfo.birthDate = req.body.birthDate;
...
function parseResetPasswordArg(req)
{
	...
	else if (!req.body.newPassword || req.body.newPassword.length < 5 || !/^[a-z0-9éèàêô,'!@# -]+$/i.test(req.body.newPassword))
...
exports.resetPassword = async (req, res, next) => {
	try {
		...
		if (!user)
			throw new Error('Error: Invalid user id!');
		if (!await bcrypt.compare(req.body.currentPassword, user.userInfo.password))
			throw new Error('Invalid current password!');
...
function parseAttachGoogleAccountErrors(res, err)
{
	...
	} else if (err.message == "Auth with Google failed, invalid token or client id!") {
		res.status(400).json({
			code: '1',
			message: err.message
		})
