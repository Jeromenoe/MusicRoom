...
const {ActivateAccount} = require('../models/User');
const {LoginObject} = require('../models/User');
...
const CLIENT_ID = "524786201017-giagchb12v60sefkfvhp0vbldap7hfak.apps.googleusercontent.com";
...
async function sendValidationMail(user, activationKey)
{
	return new Promise((resolve, reject)=>{
		const mailData = {
			...
			text: '',
			html: '<b>Hey ' + user.userInfo.firstName + '</b> \
			 	<br>Click http://62.34.5.191:45559/api/auth/activate/' + user._id + '/'
			 	+ activationKey + ' to activate your MusicRoom account.<br/>'
		};
...
async function createUserFromGoogle(req) {
	...
	const userInfo = new UserInfo({
		...
		pseudo: payload.given_name + require('crypto').randomBytes(5).toString('hex'),
		email: payload.email,
...
async function createUserFromSource(req)
{
	...
	if (req.url.indexOf('/signup/google') != -1) {
		user = await createUserFromGoogle(req)
		.catch((err) => {
			throw {
				message: err.message,
				source: 'google'
		...
		user = await createUserFromFacebook(req)
		.catch((err) => {
			throw {
				message: err.message,
				source: 'facebook'
...
exports.signup = async (req, res, next) => {
	try {
		...
		if (!user.userInfo.active)
			await sendValidationMail(user, activationKey);
...
function parseLoginErrors(req, res, err)
{
	if (err.source && err.source === 'google') {
		...
		if (err.message == 'Missing email or password field!') {
			...
			if (req.url.indexOf('/login/google') != -1 || req.url.indexOf('/login/facebook') != -1)
				res.status(400).json({ code: '0'});
			else
				res.status(400).json({ code: '1'});
			...
			res.status(400).json({ code: '3'});
		} else if (err.message == 'User account temporarily blocked, try again in 5 minutes!') {
...
async function retrieveUserFromSource(req)
{
	...
	user = await User.find( { $or: [ {'userInfo.email': email}, {'userInfo.secondaryEmail': email} ] })
	return user[0];
}
...
exports.login = async (req, res, next) => {
	try {
		...
		else if (user.userInfo.login && user.userInfo.login.tryPasswordTimes >= 5 && new Date().getTime() - user.userInfo.login.lastLoginDate.getTime() < 5*60*1000) {
			console.log((new Date().getTime() - user.userInfo.login.lastLoginDate.getTime()) / 1000 / 60)
			throw new Error('User account temporarily blocked, try again in 5 minutes!');
		}
		else if ((req.url == '/login' || req.url == '/login/') && !(await bcrypt.compare(req.body.password, user.userInfo.password))) {
	...
	User.findOne({ 'userInfo.email': req.body.email })
		.then(user => {
			...
			let mailData = {
				...
				text: "Hello, you asked us to reset your password. You can now connect on MusicRoom with this password : " + password, 
				html: "Hello, you asked us to reset your password. You can now connect on MusicRoom with this password : " + password, 
			  };
...
function parseActivateErrors(res, err)
{
	...
	if (err.message == 'Invalid user id, invalid url!') {
		...
		res.status(400).json({ code: '2' });
	} else if (err.message == 'Url hash id do not match with DB hash!, invalid url!') {
		res.status(400).json({ code: '3' });
	} else {
...
exports.activate = async (req, res, next) => {
	try {
		...
		if (!user)
			throw new Error('Invalid user id, invalid url!');
		if (user.userInfo.active == true)
			throw new Error('User account is already active!');
		let activeAccount = await ActivateAccount.findOne( { userId: req.params.userId } );
