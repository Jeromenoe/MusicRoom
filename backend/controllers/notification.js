...
exports.getPlaylistsInvitations = async (req, res, next) => {
	try {
		let user = await User.findOne({ _id: req.params.userId }, {'userData.notifications.playlistsInvitations': 1})
			.populate('userData.notifications.playlistsInvitations.friend', ['_id', 'userInfo.pseudo'])
			.populate('userData.notifications.playlistsInvitations.playlist', ['_id', 'name']);
		if (!user)
			handleError(400, 0, "Couldn't find the user!");
		let playlistsInvitations = [];
		for (playlistInvitation of user.userData.notifications.playlistsInvitations) {
		...
		res.status(200).json({ code: '0', playlistsInvitations: playlistsInvitations })
	} catch(error) {
...
exports.getEventsInvitations = async (req, res, next) => {
	try {
		let user = await User.findOne({ _id: req.params.userId }, {'userData.notifications.eventsInvitations': 1})
			.populate('userData.notifications.eventsInvitations.friend', ['_id', 'userInfo.pseudo'])
			.populate('userData.notifications.eventsInvitations.event', ['_id', 'name']);
		if (!user)
			handleError(400, 0, "Couldn't find the user!");
...
exports.getFriendsInvitations = async (req, res, next) => {
	try {
		let user = await User.findOne({ _id: req.params.userId }, {'userData.notifications.friendsInvitations': 1 })
			.populate('userData.notifications.friendsInvitations.user', ['_id', 'userInfo.pseudo'])
		if (!user)
			handleError(400, 0, "Couldn't find the user!");
