...
exports.getUserFriendsList = async (req, res, next) => {
    try {
        ...
        if (!user)
            throw new Error('Error: Invalid friend id!');
        let friends = [];
...
function parseDeleteFriendErrors(res, err)
{
    ...
    } else if (err.message == "The friend ID specified does not match any of your friends!") {
        res.status(400).json({
            code: '1',
            message: err.message
        })
...
async function deleteFriendFromUserData(userId, friendId)
{
    ...
    if (friendIdx == -1)
        throw new Error('The friend ID specified does not match any of your friends!');
    user.userData.friendsId.splice(friendIdx, 1);
    friendIdx = user.userData.discussions.map( function(e) { return e.recipientId; } ).indexOf(friendId)
    if (friendIdx != -1) {
...
function parseInviteFriendErrors(res, err)
{
    ...
    } else if (err.message == "User already has this friend in his friends list!") {
        res.status(400).json({
            code: '2',
            message: err.message
        })
    } else if (err.message == "User already has a pending invitation with this friend!") {
        res.status(400).json({
            code: '3',
            message: err.message
        })
...
async function sendInvitationToFriendData(req)
{
    ...
    if (friend.userData.notifications.friendsInvitations.map( function(e) { return e.user; } ).indexOf(req.params.userId) != -1)
        throw new Error('User already has a pending invitation with this friend!');
    if (user.userData.notifications.friendsInvitations.map( function(e) { return e.user; } ).indexOf(req.params.friendId) != -1)
        throw new Error('User already has a pending invitation with this friend!');
...
async function acceptInvitationFromUser(req)
{
    ...
    if (user.userData.friendsId.indexOf(req.params.friendId) != -1)
        throw new Error('User already has this friend in his friends list!');
    let friendIdx = user.userData.notifications.friendsInvitations.map( function(e) { return e.user; } ).indexOf(req.params.friendId);
    if (friendIdx == -1)
...
async function deleteInvitationFromUser(userId, friendId)
{
    ...
    if (!friend)
        throw new Error('Invalid friend id!');
    let friendIdx = user.userData.notifications.friendsInvitations.map(function(e) { return e.user}).indexOf(friendId);
    if (friendIdx == -1)
        throw new Error('No pending invitation from friend!');
