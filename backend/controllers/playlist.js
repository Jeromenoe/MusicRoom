...
const followPlaylistHandleError = (user, playlist) => {
	...
	if (!playlist)
		handleError(400, 1, "Couldn't find the playlist!");
	if (!playlist.public)
		handleError(400, 2, "Playlist is private!");
...
const sendInvitationToPlaylistHandleError = (playlist, user, friend) => {
	if (!playlist)
		handleError(400, 0, "Couldn't find the playlist!");
	if (!user)
		handleError(400, 1, "Couldn't find the user!");
	if (!friend)
		handleError(400, 2, "Couldn't find the friend!");
	if (!playlist.creator.equals(user._id))
		handleError(400, 3, "You don't have the right to invite someone to this playlist!");
	for (friendPlaylist of friend.userData.playlists) {
		if (friendPlaylist.playlist.equals(playlist._id) && 
				friendPlaylist.playlistType == ASSOCIATED)
			handleError(400, 4, "The friend is already associated to this playlist!");
	}
	for (friendPendingInvitation of friend.userData.notifications.playlistsInvitations) {
		if (friendPendingInvitation.playlist.equals(playlist._id) 
				&& friendPendingInvitation.friend.equals(user._id))
			handleError(400, 5, "The friend has already a pending invitation to this playlist!");
	}
	let isFriend = false;
	for (userFriend of user.userData.friendsId) {
		if (userFriend.equals(friend._id))
			isFriend = true;
	}
	if (!isFriend)
		handleError(400, 6, "The friend is not in your friends list!");	
}
...
const acceptInvitationToPlaylistHandleError = (user, playlist) => {
	...
	for (userPendingInvitation of user.userData.notifications.playlistsInvitations) {
	...
	if (!playlistInvitation)
		handleError(400, 2, "The user doesn't have a pending invitation to this playlist!");
	for (userPlaylist of user.userData.playlists) {
		if (userPlaylist.playlist.equals(playlist._id) && 
				userPlaylist.playlistType == ASSOCIATED)
...
const removeAssociatedUserFromPlaylistHandleError = (user, associatedUser, playlist) => {
	if (!playlist)
		handleError(400, 0, "Couldn't find the playlist!");
	if (!user)
		handleError(400, 1, "Couldn't find the user!");
	if (!associatedUser)
		handleError(400, 2, "Couldn't find the associatedUser!");
	...
	if (!isAssociated)
		handleError(400, 5, "The associatedUser is not associated to this playlist!");
}

const changeEditionRightAssociatedUserHandleError = (playlist, user, associatedUser) => {
	if (!playlist)
		handleError(400, 0, "Couldn't find the playlist!");
	if (!user)
		handleError(400, 1, "Couldn't find the user!");
	...
	if (!isAssociated)
		handleError(400, 4, "The associatedUser is not associated to this playlist!");
}
...
exports.getPlaylistTracks = async (req, res, next) => {
	try {
		let playlist = await Playlist.findOne({ _id: req.params.playlistId }, {"tracks.trackId":1});
		if (!playlist)
			handleError(400, 0, "Couldn't find the playlist!");
		let trackArray = []
...
exports.getAllUserPlaylists = async (req, res, next) => {
	try {
		...
		.populate({ path: 'userData.playlists.playlist', populate: { path: 'creator', select :['_id', 'userInfo.pseudo']}})
		user.userData.playlists.sort((a, b) => { return a.playlist.name.localeCompare(b.playlist.name, 'fr', {ignorePunctuation: true}); });
		if (!user)
			handleError(400, 0, "Couldn't find the user!");
		res.musicRoomMessage = "Ok"
...
exports.getUserFollowedPlaylists = async (req, res, next) => {
	try {
		let user = await User.findOne({ _id: req.params.userId }).populate('userData.playlists.playlist');
		if (!user)
			handleError(400, 0, "Couldn't find the user!");
...
exports.sendInvitationToPlaylist = async (req, res, next) => {
	try {
		...
		if (req.body.editionRight)
			editionRight = (req.body.editionRight == 'true' || req.body.editionRight == 1);
		playlistInvitation.editionRight = editionRight;
		friend.userData.notifications.playlistsInvitations.push(playlistInvitation);
		await friend.save();
		res.musicRoomMessage = "Ok"
		res.status(200).json({ code: '0', message: 'Invitation to the playlist sent to the friend!' })
	} catch(error) {
        res.musicRoomMessage = error.message
		catchErrors(error, res);
	}
...
exports.acceptInvitationToPlaylist = async (req, res, next) => {
	try {
		...
		user.userData.notifications.playlistsInvitations.pull(pendingInvitation._id);
...
exports.removeAssociatedUserFromPlaylist = async (req, res, next) => {
	try {
		let playlist = await Playlist.findOne({ _id: req.params.playlistId });
		let user = await User.findOne({ _id: req.params.userId });
		let associatedUser = await User.findOne({ _id: req.params.associatedUserId });
		...
		await Promise.all(
			[playlist, associatedUser].map( m => m.save() )
		);
		res.musicRoomMessage = "Ok"
...
exports.switchEditionRight = async (req, res, next) => {
	try {
		...
		if (playlist.creator != req.params.userId)
			handleError(400, 1, "The user doesn't have the rights to change the edition's right!");
		if (playlist.public == false && playlist.editionRight == false) 
			handleError(400, 2, "The playlist is private!");
		await Playlist.updateOne({ _id: req.params.playlistId }, {$set: {editionRight: !playlist.editionRight}})
		res.musicRoomMessage = "Ok"
		if (playlist.editionRight)
			res.status(200).json({ code: '0', message: "Edition right set to 'false'!", editionRight: 0});
		else
			res.status(200).json({ code: '1', message: "Edition right set to 'true'!", editionRight: 1});
	} catch(error) {
        res.musicRoomMessage = error.message
		catchErrors(error, res);
	}
...
exports.switchEditionRightAssociatedUser  = async (req, res, next) => {
	try {
		...
		for (playlistAssociatedUser of playlist.associatedUsers) {
			if (playlistAssociatedUser.userId.equals(associatedUser._id)) {
				playlistAssociatedUser.editionRight = !playlistAssociatedUser.editionRight;
		...
		if (!associatedUserEditionRight)
			res.status(200).json({ code: '0', message: "Edition right of the associated set to 'false'!", editionRight: '0'});
		else
			res.status(200).json({ code: '1', message: "Edition right of the associated set to 'true'!", editionRight: '1'});
	} catch(error) {
        res.musicRoomMessage = error.message
		catchErrors(error, res);
	}
...
exports.setPlaylistPublic = async (req, res, next) => {
	try {
		...
		if (!playlist)
			handleError(400, 0, "Couldn't find the playlist!");
		if (playlist.creator != req.params.userId)
			handleError(400, 1, "The user doesn't have the rights to set this playlist public!");
		if (playlist.public == true) 
			handleError(400, 2, "The playlist is already public!");
		await Playlist.updateOne({ _id: req.params.playlistId }, {$set: {public: true}})
		res.musicRoomMessage = "Ok"
...
exports.setPlaylistPrivate = async (req, res, next) => {
	try {
		...
		if (!playlist)
			handleError(400, 0, "Couldn't find the playlist!");
		if (playlist.creator != req.params.userId)
			handleError(400, 1, "The user doesn't have the rights to set this playlist private!");
		if (playlist.public == false) 
			handleError(400, 2, "The playlist is already private!");
		await Promise.all(
			[Playlist.updateOne({ _id: req.params.playlistId }, {$set: {public: false, followers: [], editionRight: false}}),
			User.updateMany({ }, { $pull: { "userData.playlists": { "playlist": req.params.playlistId, playlistType: FOLLOWED }}})]
		)
...
exports.changeMusicalStyle = async (req, res, next) => {
	try {
		...
		if (!playlist)
			handleError(400, 0, "Couldn't find the playlist!");
		if (playlist.creator != req.params.userId)
			handleError(400, 1, "The user doesn't have the rights to change the musical rights!");
		if (!req.body.musicalStyle || !musicalStylesAllowed.includes(req.body.musicalStyle))
			handleError(400, 2, "The musical style isn't valid!");
		playlist.musicalStyle = req.body.musicalStyle;
		await playlist.save();
		res.musicRoomMessage = "Ok"
		res.status(200).json({ code: '0', message: 'Playlist musical style changed!', musicalStyle: playlist.musicalStyle});
	} catch(error) {
        res.musicRoomMessage = error.message
		catchErrors(error, res);
	}
...
exports.createPlaylist = async (req, res, next) => {
	try {
		...
		res.musicRoomMessage = "Ok"
		res.status(201).json({ code: '0', message: 'Playlist created!', playlistId: playlist._id})
	} catch(error) {
        res.musicRoomMessage = error.message
		catchErrors(error, res);
	}
...
exports.deletePlaylist = async (req, res, ext) => {
	try {
		...
		if (!playlist)
			handleError(400, 0, "Couldn't find the playlist!");
		if (playlist.creator != req.params.userId) 
			handleError(400, 1, "The user doesn't have the rights to delete this playlist!");
		await Promise.all(
			[Playlist.deleteOne({ _id: req.params.playlistId }),
			User.updateMany({ $pull: { "userData.playlists": { "playlist": req.params.playlistId }}})]
		)
		res.musicRoomMessage = "Ok"
...
exports.addTrack = async (req, res, next) => {
	try {
		...
		if (playlist.editionRight != 1 && !playlist.creator.equals(user._id) 
				&& !associatedUserEditionRight)
			handleError(400, 2, "The user doesn't have the rights to add track in this playlist!");
...
exports.deleteTrack = async (req, res, next) => {
	try {
		...
		if (playlist.editionRight != 1 && !playlist.creator.equals(user._id) 
				&& !associatedUserEditionRight)
			handleError(400, 2, "The user doesn't have the rights to delete track in this playlist!");
