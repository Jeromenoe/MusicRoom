...
const UrlHandleError = (req) => {
	...
	if (!req.query.value || req.query.value == undefined) 
		req.query.value = ""
	if (!req.query.musicalStyle || req.query.musicalStyle == undefined 
		|| !musicalStylesAllowed.includes(req.query.musicalStyle)) {
			req.query.musicalStyle = ""
		}
	if (!req.query.type)
		handleError(400, 2, "type must be specified")
	if (req.query.limit) {
...
var searchPseudos = async function(parameters)  {
	...
	let users;
	if (parameters.limit)
		users = await User.find({'userInfo.pseudo' : {$regex: regex}}, {'userInfo.pseudo': 1}).limit(parameters.limit)
	else
		users = await User.find({'userInfo.pseudo' : {$regex: regex}}, {'userInfo.pseudo': 1})
	users.sort((a, b) => { return a.userInfo.pseudo.localeCompare(b.pseudo, 'fr', {ignorePunctuation: true}); });
	return users
}
...
const searchPlaylists = async (parameters) => {
	...
	let playlists;
	if (parameters.limit) {
		playlists = await Playlist.find({'name' : {$regex: regex}, public: true, 'musicalStyle': {$regex: styleRegex}}, {'name': 1, 'musicalStyle': 1})
		.populate('creator').limit(parameters.limit);
	}
	else {
		playlists = await Playlist.find({'name' : {$regex: regex}, public: true, 'musicalStyle': {$regex: styleRegex}}, {'name': 1, 'musicalStyle': 1})
		.populate('creator');
	}
	playlists.sort((a, b) => { return a.name.localeCompare(b.name, 'fr', {ignorePunctuation: true}); });
	let newPlaylist = [];
...
const searchEvents = async (parameters) => {
	...
	let events;
	if (parameters.limit) {
		events = await Event.find({'name' : {$regex: regex}, publicFlag: true, 'musicalStyle': {$regex: styleRegex}}, {'name': 1, 'musicalStyle': 1})
		.populate('creator').limit(parameters.limit);
	}
	else {
		events = await Event.find({'name' : {$regex: regex}, publicFlag: true, 'musicalStyle': {$regex: styleRegex}}, {'name': 1, 'musicalStyle': 1})
	...
	events.sort((a, b) => { return a.name.localeCompare(b.name, 'fr', {ignorePunctuation: true}); });
	let newEvents = [];
...
exports.searchFriends = async (req, res, next) => {
	try {
		...
		let regex = new RegExp(regexString, 'i');
		let user = await User.findOne({ _id: req.params.userId }, {'userData.friendsId': 1})
