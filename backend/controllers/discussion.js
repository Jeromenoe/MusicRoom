...
const {ObjectMsg} = require('../models/User')
const socketIO = require('../server')
const eventCtrl = require('./event')

function sendMsg(ownerId, ownerPseudo, recipientId, content) {
    socketIO.io.sockets.in(recipientId).emit("receiveMsg", {ownerId: ownerId, ownerPseudo: ownerPseudo, content: content}); //add data
}
...
async function storeMsgInOwnerDB(owner, recipient, content) {
    ...
    let discussionIdx = owner.userData.discussions.map(function(e) { return e.recipientId; }).indexOf(recipient._id)
    if (discussionIdx == -1) {
...
async function storeMsgInRecipientDB(owner, recipient, content) {
    let discussionIdx = recipient.userData.discussions.map(function(e) { return e.recipientId; }).indexOf(owner._id)
    if (discussionIdx == -1) {
...
exports.getAllMyDiscussions = async(req, res, next) => {
    try {
        let user = await User.findOne({_id: eventCtrl.getUserIdFromToken(req)})
